...
/// Represents a variable captured by a lambda during canonicalization
const CapturedVariable = struct {
    name: base.Ident.Idx,
    pattern_idx: CIR.Pattern.Idx,
    source_function_context: u32,
};
...
const FunctionContext = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, depth: u32) FunctionContext {
        return FunctionContext{
            .depth = depth,
            .captures = std.ArrayList(CapturedVariable).init(allocator),
        };
    }
    ...
    switch (expr) {
                ...
                if (context.captures.items.len > 0) {
                    ...
                    for (context.captures.items, 0..) |capture, i| {
                        captured_vars[i] = CIR.Expr.CapturedVar{
                            ...
                            .scope_depth = capture.source_function_context,
                        };
                    }

                    break :blk CIR.Expr.CaptureInfo{
                        .captured_vars = captured_vars,
                        .capture_pattern_idx = null, // TODO: implement capture record pattern optimization
                    };
                } else {
                    break :blk CIR.Expr.CaptureInfo.empty;
                }
            };
        ...
        .match => |m| {
            ...
            for (branches_slice, 0..) |ast_branch_idx, index| {
                ...
                const branch_idx = try self.can_ir.addMatchBranchAndTypeVar(CIR.Expr.Match.Branch{
...
fn recordCapture(self: *Self, name: base.Ident.Idx, pattern_idx: CIR.Pattern.Idx, source_depth: u32) !void {
    ...
    try current_context.captures.append(CapturedVariable{
        .name = name,
        .pattern_idx = pattern_idx,
        .source_function_context = source_depth,
    });
}
